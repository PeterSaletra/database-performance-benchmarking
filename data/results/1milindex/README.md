...
To generate the same charts without ScyllaDB results:

```bash
python src/plot_results.py --input data/results/benchmark_<run_id>.csv --no-scylla --output-dir plots
```

To generate charts for all benchmark CSVs found in `data/results`:

```bash
python src/plot_results.py --all --output-dir plots
```

And all charts at once without ScyllaDB:

```bash
python src/plot_results.py --all --no-scylla --output-dir plots
```

To generate charts in pairs (MongoDB vs ScyllaDB, and PostgreSQL vs MySQL):

```bash
python src/plot_results.py --input data/results/benchmark_<run_id>.csv --pairs --output-dir plots
```

The paired mode also works with `--all`, comparison mode (`--baseline-input` + `--after-input`), and can be combined with `--no-scylla`.
...
The comparison mode also supports `--no-scylla`.
